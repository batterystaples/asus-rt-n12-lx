...
#include <stdbool.h>  // Alicia, 201103
...
void tftpd_send_ack_asus(Int16 number);//Lucifer
...
//----------------------------------------------------------------------------------------
static void doARPReplyAsus(void)//Lucifer
{
 ...
 if( udpheader->dest==htons(TFTP_PORT) )
  ...
  //  memcpy(arptable_tftp[TFTP_CLIENT].node,(Int8*)&(nic.packet[ETH_ALEN]), ETH_ALEN); 
   //Lucifer
	...
	if(!memcmp(filename,"ASUSSPACELINK",13))
		{
			prom_printf("\n**filename: %02X %02X %02X %02X Byte\n",filename[13],filename[14],filename[15],filename[16]);
			arptable_tftp[TFTP_SERVER].ipaddr.s_addr=IPTOUL(((unsigned char)filename[16]),((unsigned char)filename[15]),((unsigned char)filename[14]),((unsigned char)filename[13]));
			doARPReplyAsus();
		}//Lucifer
    one_tftp_lock=1;
...
// Alicia, 201103 {
	...
	int order_flag = 0;
	bool linux_bin_first = false;
	bool root_bin_second = false;
	bool is_firmware = false;

	while (head_offset < len)
	{
		memcpy(&Header, ((char *)startAddr + head_offset), sizeof(IMG_HEADER_T));
		if ((order_flag == 0) && (!memcmp(Header.signature, FW_SIGNATURE, SIG_LEN) 
			|| !memcmp(Header.signature, FW_SIGNATURE_WITH_ROOT, SIG_LEN)))
		{
			linux_bin_first = true;
		}
		
		if ((order_flag == 1) && !memcmp(Header.signature, ROOT_SIGNATURE, SIG_LEN) && linux_bin_first)
		{
			root_bin_second = true;
		}

		head_offset += Header.len + sizeof(IMG_HEADER_T);
	   	order_flag++;
//prom_printf("\n####Header.signature: %s, head_offset: %ld(0x%x), len: %ld \n", Header.signature, head_offset, head_offset, len);
	}
//prom_printf("\n######order_flag: %d, linux_bin_first: %d, root_bin_second: %d\n", order_flag, linux_bin_first, root_bin_second);
	if ((order_flag == 2) && root_bin_second)
	{
		is_firmware = true;
//prom_printf("\n####is_firmware: %d\n", is_firmware);
	}
...
// Alicia, 201103 }
	...
	while ((head_offset + sizeof(IMG_HEADER_T)) < len){
		...
		if(skip_check_signature || 
			...
			for (j=0; j<6; j++)//Lucifer
			{
				tftpd_send_ack_asus((Int16)0);//Lucifer
				for(k=0;k<100000;k++)//Lucifer
				;
			}
				return ;
			}
...
// Alicia, 201103 {
		...
		if(Header.burnAddr+burn_offset+burnLen > spi_flash_info[0].chip_size)
		{
			...
			if(spi_flw_image_mio_8198(0,Header.burnAddr+burn_offset, srcAddr, burnLen))
				trueorfaulse = 1;
#else
		unsigned long flash_burnAddr = 0;
		if (memcmp(Header.signature, ROOT_SIGNATURE, SIG_LEN))
			flash_burnAddr = Header.burnAddr + burn_offset;
		else  // root.bin
			flash_burnAddr = 0x10000 + head_offset + burn_offset;
prom_printf("\n####head_offset: %ld(0x%x), flash_burnAddr: 0x%x, is_firmware: %d\n", head_offset, head_offset, flash_burnAddr, is_firmware);
		if (!is_firmware && (flash_burnAddr >= 0x10000))
		{
prom_printf("\n####It is not firmware image or your flash burn address is incorrect!\n");
			return;
		}
prom_printf("\n####Start to burn ...\n");
		if(flash_burnAddr+burnLen > spi_flash_info[0].chip_size)
		{
			if(spi_flw_image_mio_8198(0,flash_burnAddr, srcAddr, spi_flash_info[0].chip_size-(flash_burnAddr))&&
				spi_flw_image_mio_8198(1,0, srcAddr+spi_flash_info[0].chip_size-(flash_burnAddr), flash_burnAddr+burnLen-spi_flash_info[0].chip_size))
				trueorfaulse = 1;
		}
		else
			if(spi_flw_image_mio_8198(0,flash_burnAddr, srcAddr, burnLen))
				trueorfaulse = 1;
#endif
// Alicia, 201103 }
#else
	...
	for (j=0; j<6; j++)//Lucifer
	{
		tftpd_send_ack_asus((Int16)1);//Lucifer
		for(k=0;k<100000;k++)//Lucifer
		;	
	}
	if(reboot){
...
static void prepareACK(void)
{
 ...
 char block_received_modelname[5]={0}; /*Edison 2011.5.26*/
#ifdef CONFIG_NFBI
    IMG_HEADER_T header;
    int ret;
    ...
    if(block_received != (block_expected))
    {
     ...
     {      
      ...
      /*put the image into memory address*/
      	...
      	memcpy(block_received_modelname,tftppacket->u.data.download,4); /*Edison 2011.5.26*/
     
      if(block_received ==1 && !strcmp(block_received_modelname,"RTLK")){	/*Edison 2011.5.26*/
      	memcpy((void *)address_to_store, tftppacket->u.data.download+60, tftpdata_length-60);	
	address_to_store=address_to_store+tftpdata_length-60;
	/*use this to count the image bytes*/
	file_length_to_server=file_length_to_server+tftpdata_length-60;
      ...
      if(tftpdata_length < TFTP_DEFAULTSIZE_PACKET)
        {
         ...
         /*Cyrus Tsai*/
         one_tftp_lock=0; 
       //  SERVER_port++;//this maybe useless Lucifer for asus
...
static void prepareDATA(void)
{
 ...
 Int16 block_received_tmp=0;
char block_received_modelname[60]={0};
int i;
    if (!tftpd_is_ready)
        return;
 udpheader = (struct udphdr *)&nic.packet[ETH_HLEN+ sizeof(struct iphdr)];
...
void tftpd_send_ack_asus(Int16 number)//Lucifer
{
 ...
 tftp_tx.opcode=htons(6);//Lucifer
 ...
 swNic_txDone();//Lucifer for clear descriptor
}
//----------------------------------------------------------------------------------------
void tftpd_send_ack(Int16 number)
{
...
if (512*block_number==(file_length_to_client+512)||!memcmp(filename,"ASUSSPACELINK",13))
    {
    /*it is over that means a length=0 data is required*/
     prom_printf("run ASUSSPACELINK\n");
    length=0;
    it_is_EOF=1;
    }
        ...
        case htons(FRAME_IP):
            ...
            if (nic.packetlen > UDPIPETHheader) {
                ...
                if (ipheader->verhdrlen==0x45) {
                    ...
                    if (ip_addr.s_addr==arptable_tftp[TFTP_SERVER].ipaddr.s_addr || ip_addr.s_addr == 0xFFFFFFFF //Lucifer for rrq brocast
