...
// Alicia, 201103 {
	...
	if(ret == 2)
    	{
prom_printf("\n####return_addr: 0x%x, root_bin_offset: 0x%x\n", return_addr, (unsigned long)return_addr + sizeof(IMG_HEADER_T) + pHeader->len);
    		ret = check_rootfs_image((unsigned long)return_addr + sizeof(IMG_HEADER_T) + pHeader->len);
	}
...
int user_interrupt(unsigned long time)
{
	...
	do {
		...
		if(ret == 1) 
		{
		monitor_chose=1;//Lucifer
		return 1;
		}
		ret=pollingDownModeKeyword(46);//Lucifer :46 is the value of "del" key
		{
		if(ret == 1) return 1;
		}
#ifdef CONFIG_BOOT_RESET_ENABLE		
