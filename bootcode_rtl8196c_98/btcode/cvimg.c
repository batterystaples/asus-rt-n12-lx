 ...
 *
 *      $Id: cvimg.c,v 1.1 2011/03/23 04:09:53 jerry_jian Exp $
 *
 */
...
{
    //printf("Version: 1.1\n");
    printf("Usage: cvimg boot input-filename output-filename start-addr burn-addr version\n");
...
int main(int argc, char** argv)
{
	...
	if (is_vmlinuxhdr) {
		...
		if( !strcmp("boot", argv[1]))
			size = (24*1024) + sizeof(IMG_HEADER_T);
		else
		{
	...
	if (is_vmlinuxhdr) {
		...
		*((unsigned int *)((char *)pHeader+4)) = start_addr;
		...
		else if ( !strcmp("boot", argv[1]))
		{
			...
			sscanf(argv[6], "%d.%d.%d.%d", &ver1, &ver2, &ver3, &ver4);
			*((unsigned short *)&buf[size-sizeof(IMG_HEADER_T)-sizeof(checksum)-4]) = ver1;
			*((unsigned short *)&buf[size-sizeof(IMG_HEADER_T)-sizeof(checksum)-3]) = ver2;
			*((unsigned short *)&buf[size-sizeof(IMG_HEADER_T)-sizeof(checksum)-2]) = ver3;
			*((unsigned short *)&buf[size-sizeof(IMG_HEADER_T)-sizeof(checksum)-1]) = ver4;
			//2011.03.23 Jerry }
		}		
		...
		//2011.03.22 Jerry {
		if ( !strcmp("boot", argv[1]))
			checksum = WORD_SWAP(calculateChecksum(buf, (24*1024)-sizeof(checksum) ));
		else
