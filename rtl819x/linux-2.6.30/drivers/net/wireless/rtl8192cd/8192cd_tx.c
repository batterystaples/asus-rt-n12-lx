 ...
 *  TX handle routines
 *
 *  $Id: 8192cd_tx.c,v 1.2 2011/05/16 09:57:01 edison_shih Exp $
 *
...
int mlcst2unicst(struct rtl8192cd_priv *priv, struct sk_buff *skb)
{
	...
	if(	((skb->data[0]==0x01) && (skb->data[1]==0x00) && (skb->data[2]==0x5e) 
		 && (skb->data[3]==0x00) && (skb->data[4]==0x00)&& (skb->data[5]==0x01))
		 ||
		 ((skb->data[0]==0x33) && (skb->data[1]==0x33) && (skb->data[2]==0x00) 
		 && (skb->data[3]==0x00) && (skb->data[4]==0x00)&& (skb->data[5]==0x01))
		)
	{
	...
	while (phead && (plist != phead)) {
		...
		for (i=0; i<MAX_IP_MC_ENTRY; i++) {
			if (pstat->ipmc[i].used && !memcmp(&pstat->ipmc[i].mcmac, origDest, 6)) {
				pstat_found = pstat;
				iffind++;
				break;
			}
...
__MIPS16
__IRAM_IN_865X
int __rtl8192cd_start_xmit_out(struct sk_buff *skb, struct net_device *dev, int tx_flag);
...
void rtl8192cd_beq_timer(unsigned long task_priv)
{
	...
	while(1)
        {
                skb = skb_dequeue(&pstat->be_queue);

                if (skb == NULL)
                        break;
                __rtl8192cd_start_xmit_out(skb, skb->dev, TX_NORMAL);
        }
	pstat->be_timeout_cnt++;
}
...
void rtl8192cd_bkq_timer(unsigned long task_priv)
{
        ...
        while(1)
        {
                skb = skb_dequeue(&pstat->bk_queue);

                if (skb == NULL)
                        break;
                __rtl8192cd_start_xmit_out(skb, skb->dev, TX_NORMAL);
        }
        pstat->bk_timeout_cnt++;
}
...
void rtl8192cd_viq_timer(unsigned long task_priv)
{
        ...
        while(1)
        {
                skb = skb_dequeue(&pstat->vi_queue);

                if (skb == NULL)
                        break;
                __rtl8192cd_start_xmit_out(skb, skb->dev, TX_NORMAL);
        }
        pstat->vi_timeout_cnt++;
}
...
void rtl8192cd_voq_timer(unsigned long task_priv)
{
        ...
        while(1)
        {
                skb = skb_dequeue(&pstat->vo_queue);

                if (skb == NULL)
                        break;
                __rtl8192cd_start_xmit_out(skb, skb->dev, TX_NORMAL);
        }
        pstat->vo_timeout_cnt++;
}
#endif
...
int __rtl8192cd_start_xmit(struct sk_buff *skb, struct net_device *dev, int tx_flag)
{
	...
	{
		protocol = ntohs(*((UINT16 *)((UINT8 *)skb->data + (ETH_ALEN<<1))));

		if ((passThruStatusWlan) && 
			( ((passThruStatusWlan&IP6_PASSTHRU_MASK)&&(protocol == __constant_htons(ETH_P_IPV6))) || 
				((passThruStatusWlan&PPPOE_PASSTHRU_MASK)&&((protocol==__constant_htons(ETH_P_PPP_SES))||
				(protocol==__constant_htons(ETH_P_PPP_DISC)))) ) )
		{
...
//mark_test , remove EAP padding by ethernet
	...
	if ((pstat != NULL) && (priv->swq_en == 1) && is_MCS_rate(pstat->current_tx_rate))
        {
                {
                        int q_len, aggr_len, pri, q_num;
			...
			if (q_num == VO_QUEUE)
			{
				pqueue = &pstat->vo_queue;
				ptimer = &pstat->voq_timer;
				timer_hook = rtl8192cd_voq_timer;
			}
			else if (q_num == VI_QUEUE)
			{
				pqueue = &pstat->vi_queue;
				ptimer = &pstat->viq_timer;
				timer_hook = rtl8192cd_viq_timer;
			}
			else if (q_num == BE_QUEUE)
			{
				pqueue = &pstat->be_queue;
				ptimer = &pstat->beq_timer;
				timer_hook = rtl8192cd_beq_timer;
			}
			else
			{
				pqueue = &pstat->bk_queue;
				ptimer = &pstat->bkq_timer;
				...
				while(1)
                                {
                                        ...
                                        if (tmpskb == NULL)
                                                break;
                                        __rtl8192cd_start_xmit_out(tmpskb, tmpskb->dev, TX_NORMAL);
                                }
				goto send_out;
			}
...
send_out:
                __rtl8192cd_start_xmit_out(skb, dev, tx_flag);
        }
