...
void UpdateBBRFVal8192DE(struct rtl8192cd_priv *priv)
{
	...
	if (priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G) {
		...
		//1.5V_LDO
		//RTL_W32(0x14, ((RTL_R32(0x14)&0xff0fffff)|0x00800000));
	...
	//Update RF
	//After load, we know the RF type. todo :some bb regs need also  update.
		//The band is decided by radioa/b.txt??		
			//Read RF reg0x18 to get band mode.
	if (priv->pmib->dot11RFEntry.macPhyMode == DUALMAC_DUALPHY)
		curMaxRFPath = RF92CD_PATH_B;
	else
		curMaxRFPath = RF92CD_PATH_MAX;
	...
	for(eRFPath = RF92CD_PATH_A; eRFPath <curMaxRFPath; eRFPath++) {
		//printk("[%s][%d]:[0x%x]\n", __FUNCTION__, __LINE__, u4RegValue);
		u4RegValue = PHY_QueryRFReg( priv, eRFPath, rRfChannel, bMask20Bits, 1);
		// need to confirm
		if(priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G) {
			/*
			 *	5G
			 */
			u4RegValue &= 0xFFFFFF00;
			u4RegValue |= (BIT(16)|BIT(8)|0x24); //set channel 36
			PHY_SetRFReg(priv,eRFPath, rRfChannel,bMask20Bits, u4RegValue);
			delay_ms(1);			
			// LDO_DIV
			 ...
			 *	2.4G
			 */
			u4RegValue &=  ~(BIT(16)|BIT(8)|0xFF);
			u4RegValue |= 1; //st channel 1.
			PHY_SetRFReg(priv,eRFPath, rRfChannel,bMask20Bits, u4RegValue);
			delay_ms(1);
			// LDO_DIV
	...
	if (priv->pmib->dot11RFEntry.macPhyMode == DUALMAC_DUALPHY) {
		...
		if (!IS_TEST_CHIP(priv)){
			//BS_VCO
			if (priv->pshare->wlandev_idx == 0)
				PHY_SetRFReg(priv,RF92CD_PATH_A, 0x28, BIT(10)|BIT(9)|BIT(8), 7); // 3b'111
			else
				PHY_SetRFReg(priv,RF92CD_PATH_A, 0x28, BIT(10)|BIT(9)|BIT(8), 5); // 3b'101
		}
		*/
		...
		PHY_SetBBReg(priv, rFPGA0_AdDaClockEn, BIT(13) | BIT(12), 3);
	} else
#endif
	{
		/*
		if (!IS_TEST_CHIP(priv)){
			//BS_VCO
			PHY_SetRFReg(priv,RF92CD_PATH_A, 0x28, BIT(10)|BIT(9)|BIT(8), 7); // 3b'111
			PHY_SetRFReg(priv,RF92CD_PATH_B, 0x28, BIT(10)|BIT(9)|BIT(8), 5); // 3b'101
		}*/
	...
	/*
	 *	Set RxAGC at here before DIG is ready
	 */
	//MUST BE REMOVED!!
	//RTL_W8(rOFDM0_XAAGCCore1, 0x30);
	//RTL_W8(rOFDM0_XBAGCCore1, 0x30);

	//LLT table init.
	//The change between DMDP----SMSP,need it.
	//need  to do??

	//Adapter->HalFunc.SetHwRegHandler(Adapter, HW_VAR_AMPDU_FACTOR, &pHTInfo->CurrentAMPDUFactor);

	//Just for test.
	//PHY_SetBBReg(Adapter, rOFDM0_XAAGCCore1, bMaskByte0, 0x28);
	//PHY_SetBBReg(Adapter, rOFDM0_XBAGCCore1, bMaskByte0, 0x28);

	//PHY_SetBBReg(Adapter, rFPGA0_RFMOD, BIT24|BIT25, 0x3);	
...
{
	unsigned int temp_800, temp_c04, temp_874, temp_c08, temp_870, temp_860, temp_864, temp_88c, temp_c50, temp_c58,
				switch2PI=0, Oldval_0, Oldval_1, X, TX0_A, TX1_A, reg;
	u8 temp_522, temp_550, temp_551;
	unsigned int cal_num=0, cal_retry=0, ADDA_backup[IQK_ADDA_REG_NUM];
	int Y, TX0_C, TX1_C, result[8][3], result_final[8];
...
void phy_ReloadLCKSetting(struct rtl8192cd_priv *priv)
{
	...
	unsigned int 	u4tmp = 0, u4RegValue = 0;
	unsigned char	bNeedPowerDownRadio = FALSE;
	unsigned int 	channel =  PHY_QueryRFReg(priv, eRFPath, rRfChannel, 0xff, 1);

	DEBUG_INFO("====>phy_ReloadLCKSetting interface %d path %d ch %d\n", priv->pshare->wlandev_idx, eRFPath, channel);
	...
	if(priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_5G)
	{
		...
		DEBUG_INFO("%s ver 1 set RF-A, 5G,	0x28 = 0x%x !!\n",__FUNCTION__, u4tmp);
#ifdef CONFIG_RTL_92D_DMDP
		if(priv->pmib->dot11RFEntry.macPhyMode == DUALMAC_DUALPHY && priv->pshare->wlandev_idx == 1)
		{
			DMDP_PHY_SetRFReg(0,  RF92CD_PATH_A, 0x28, 0x3f800, u4tmp);
		}else
#endif
			PHY_SetRFReg(priv, eRFPath, 0x28, 0x3f800, u4tmp);	

	}
	else if(priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G)
	{
		u4tmp = CurveIndex_2G[channel-1];
		DEBUG_INFO("%s ver 3 set RF-B, 2G, 0x28 = 0x%x !!\n", __FUNCTION__, u4tmp);
#ifdef CONFIG_RTL_92D_DMDP
		if(priv->pmib->dot11RFEntry.macPhyMode == DUALMAC_DUALPHY && priv->pshare->wlandev_idx == 0)
		{
			DMDP_PHY_SetRFReg(1,  RF92CD_PATH_A, 0x28, 0x3f800, u4tmp);					
		}else
#endif
			PHY_SetRFReg(priv, eRFPath, 0x28, 0x3f800, u4tmp);
		
		DEBUG_INFO("%s ver 3 set RF-B, 2G, 0x28 = 0x%x !!\n", __FUNCTION__, PHY_QueryRFReg(priv, eRFPath, 0x28, 0x3f800, 1));
	}
...
{
	unsigned char	tmpReg, index = 0;
	unsigned int 	RF_mode[2],LC_Cal, tmpu4Byte[2];
	unsigned int	eRFPath, curMaxRFPath;
	unsigned int	i, j, smallestABSVal; //, u4tmp;
...
{
	unsigned int temp_800, temp_c04, temp_874, temp_c08, temp_870, temp_860, temp_88c, temp_c50,
				switch2PI=0, Oldval_0, X, TX0_A, reg;
	u8 temp_522, temp_550, temp_551;
	unsigned int cal_num=0, cal_retry=0, ADDA_backup[IQK_ADDA_REG_NUM];
	int Y, TX0_C, result[8][3], result_final[8];
...
int smcc_92D_enable2x2_2G(struct rtl8192cd_priv * priv)
{
	...
	// cck_enable
	DMDP_PHY_SetBBReg(0, rFPGA0_RFMOD, bCCKEn, 0x1);
	// LDO_DIV
