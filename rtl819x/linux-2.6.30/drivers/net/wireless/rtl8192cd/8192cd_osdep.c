 ...
 *
 *  $Id: 8192cd_osdep.c,v 1.2 2011/05/16 09:56:50 edison_shih Exp $
 *
...
#ifdef DOT11D
{
	...
	if(OPMODE & WIFI_P2P_SUPPORT ){
		...
		P2P_DEBUG("%s don't support P2P mode now\n",priv->dev->name);
...
__DRAM_IN_865X int passThruWanIdx;
...
int get_nic_buf_in_wireless_tx(void)
{
	int i,txCnt,counted;
	int queueCnt,idx;
	struct rtl8192cd_priv *priv;
...
struct net_device *get_shortcut_dev(unsigned char *da)
{
	...
	for (i=0; (i<sizeof(wlan_device)/sizeof(struct _device_info_)) && (dev==NULL); i++)
	{
		if (wlan_device[i].priv && netif_running(wlan_device[i].priv->dev))
		{
			...
			//hyking:for fast bridge performance
			//2010-5-10
			if (	priv->assoc_num > 0 &&
				(priv->dev->br_port) &&
