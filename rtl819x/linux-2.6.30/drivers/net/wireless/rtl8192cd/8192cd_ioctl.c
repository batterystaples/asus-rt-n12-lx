 ...
 *
 *  $Id: 8192cd_ioctl.c,v 1.2 2011/05/16 09:56:45 edison_shih Exp $
 *
...
static struct iwpriv_arg mib_table[] = {
	...
	{"smcc_t",	INT_T,	_OFFSET(dot11RFEntry.smcc_t), _SIZE(dot11RFEntry.smcc_t), 4},
#ifdef PHASE2_TEST
	{"smcc_p",	INT_T,	_OFFSET(dot11RFEntry.smcc_p), _SIZE(dot11RFEntry.smcc_p), 18},
#endif
	{"target_pwr",	INT_T,	_OFFSET(dot11RFEntry.target_pwr), _SIZE(dot11RFEntry.target_pwr), 13},
...
#ifdef DFS
			if ((strcmp(entry->name, "dfsdbgmode") == 0) 
				&& (byte_val) 
...
static int issue_mic_err_pkt(struct rtl8192cd_priv *priv, unsigned char *cli_mac)
{
	...
	if (skb != NULL) {
		...
		if (!(OPMODE & WIFI_STATION_STATE)) {
			...
			if (!memcpy(cli_mac, null_mac, MACADDRLEN)) {
...
int issue_mic_rpt_pkt(struct rtl8192cd_priv *priv, unsigned char *data)
{
	struct sk_buff *skb;
	struct wlan_ethhdr_t *pethhdr;
	int format;
...
int p2p_get_p2pconnect_state(struct rtl8192cd_priv *priv, unsigned char *data)
{
	...
	if(P2P_STATE == P2P_S_CLIENT_CONNECTED_DHCPC){
		...
		P2P_DEBUG("\n	wlan driver report client is connected\n");
		P2P_DEBUG("\n	indicate web server to start udhcpc \n\n\n");		
		P2P_STATE = P2P_S_CLIENT_CONNECTED_DHCPC_done ;	// after web rdy get this state ; change it
	}else
	if(P2P_STATE == P2P_S_preGO2GO_DHCPD){

		P2P_DEBUG("start GO mode\n");
		P2P_DEBUG("\nIndicate web server to start udhcpd ...\n\n");		
...
int p2p_wps_indicate_state(struct rtl8192cd_priv *priv, unsigned char *data)
{
	...
	if(flags == GO_WPS_SUCCESS){
		...
		{
			P2P_DEBUG("GO mdoe ;Report from wscd ; WPS is  done \n\n");	
			// mode change
			P2PMODE = P2P_TMP_GO ;
