 ...
 *  MP routines
 *
 *  $Id: 8192cd_mp.c,v 1.2 2011/05/16 09:56:46 edison_shih Exp $
 *
...
void mp_set_datarate(struct rtl8192cd_priv *priv, unsigned char *data)
{
	...
	mp_8192CD_tx_setting(priv);
	if (rate <= 108)
		sprintf(tmpbuf, "Set data rate to %d Mbps  %d\n", rate/2, (priv->pshare->mp_datarate));
	else
		sprintf(tmpbuf, "Set data rate to MCS %d   %d\n", rate&0x7f, (priv->pshare->mp_datarate));
	printk(tmpbuf);
}
