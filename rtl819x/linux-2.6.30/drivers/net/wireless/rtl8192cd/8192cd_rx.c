 ...
 *  RX handle routines
 *
 *  $Id: 8192cd_rx.c,v 1.2 2011/05/16 09:57:00 edison_shih Exp $
 *
...
#else
#if defined(CONFIG_RTL_819X_SWCORE) && defined(__LINUX_2_6__)
...
void rtl_netif_rx(struct rtl8192cd_priv *priv, struct sk_buff *pskb, struct stat_info *pstat)
{
	...
	{
		//only wisp mode, wireless client mode and ipv6-passthru enabled will go here
		protocol = ntohs(*((UINT16 *)((UINT8 *)pskb->data + (ETH_ALEN<<1))));
...
/*
	Check the "to_fr_ds" field:
						...
						ToDS = 0
*/
static int rtl8192cd_rx_dispatch_mgmt_adhoc(
	struct rtl8192cd_priv **priv_p, struct rx_frinfo *pfrinfo
#ifdef MBSSID
				,int vap_idx
#endif
				)
...
static int process_datafrme(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
	...
	if (OPMODE & WIFI_AP_STATE)
	{
		...
		if (is_qos_data(pframe)) {
			...
			{
				if (!pstat->is_realtek_sta && !pstat->is_marvell_sta && !pstat->is_ralink_sta)
...
//			if (get_free_memory() < FREE_MEM_LOWER_BOUND) {
...
#endif
			{
				...
				{
// if tkip, clone it. prevent rx MIC error
#if 1
