 ...
 *
 *  $Id: 8192cd_hw.c,v 1.2 2011/05/16 09:56:45 edison_shih Exp $
 *
...
static int get_chnl_lmt(unsigned char *line_head, unsigned int *ch_start, unsigned int *ch_end, unsigned int *limit)
{
	unsigned char *next;
	int base, idx;
	int num=0;
	...
	*ch_start = *ch_start = '\0';
	...
	if (next) {
		...
		*limit = _atoi((char *)&next[idx], base);
	}
	else
		*limit = 0;
...
void find_pwr_limit(struct rtl8192cd_priv *priv)
{
	...
	if (tmp>=0) {
		if (priv->pshare->is_40m_bw)
			priv->pshare->txpwr_lmt = priv->pshare->ch_pwr_lmt40[tmp];
		else
			priv->pshare->txpwr_lmt = priv->pshare->ch_pwr_lmt20[tmp];
	} else {
		priv->pshare->txpwr_lmt = 0;
		printk("Cannot map current working channel!\n");
	}
}
...
int PHY_ConfigTXLmtWithParaFile(struct rtl8192cd_priv *priv)
{
	...
	unsigned int  ch_start, ch_end, limit;
	unsigned char *mem_ptr, *line_head, *next_head;
	int	idx=0, tbl_idx[2], set_en=0,bw;
	priv->pshare->txpwr_lmt = 0;
	
	struct MacRegTable *reg_table = (struct MacRegTable *)priv->pshare->txpwr_lmt_buf;
	...
	tbl_idx[0] = NO_RESTRICT;
	tbl_idx[1] = NO_RESTRICT;
...
set_fs(old_fs);
	...
	while (1) {
		...
		num = get_chnl_lmt(line_head, &ch_start, &ch_end, &limit);
		if (num > 0) {
			reg_table[len].offset = ch_start;
			reg_table[len].mask = ch_end;
			reg_table[len].value = limit;
			len++;
	...
	num = 0;
	while (1) {
		ch_start = reg_table[num].offset;
		ch_end = reg_table[num].mask;
		limit = reg_table[num].value;

		//printk(">> %d-%d-%d\n",ch_start,ch_end,limit);
		if (ch_start == 0xff)
			break;
		...
		if (ch_start== 0 && ch_end == 0){
			if (limit == tbl_idx[0]) {
				set_en=1;
				bw = 1;
				memset(priv->pshare->ch_pwr_lmt20, 0, SUPPORT_CH_NUM);
			}else if (limit == tbl_idx[1]) {
				set_en=1;
				bw = 2;
				memset(priv->pshare->ch_pwr_lmt40, 0, SUPPORT_CH_NUM);
			} else {
				set_en = 0;
			}
		}
		...
		if (set_en && ch_start){
			...
			for (j=ch2idx(ch_start);j<=ch2idx(ch_end);j++){
				...
				if (bw == 1)
					priv->pshare->ch_pwr_lmt20[j] = limit;
				else if (bw==2)
					priv->pshare->ch_pwr_lmt40[j] = limit;
			}
		}
		...
		if (GET_CHIP_VER(priv) == VERSION_8192D){	
			...
			if (priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_5G){
				if (priv->pmib->dot11nConfigEntry.dot11nUse40M == 0){
					if (priv->pmib->dot11RFEntry.dot11channel<=64)
						pg_tbl_idx = AN_20_CH_36_64;
					else if (priv->pmib->dot11RFEntry.dot11channel<=140)
...
void SwChnl(struct rtl8192cd_priv *priv, unsigned char channel, int offset)
{
	...
	for(eRFPath = RF92CD_PATH_A; eRFPath < curMaxRFPath; eRFPath++)	{
		PHY_SetRFReg(priv, eRFPath, rRfChannel, 0x3ff, val);
#ifdef CONFIG_RTL_92D_SUPPORT
	if (GET_CHIP_VER(priv)==VERSION_8192D) {
		if (priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G) {
			/*
...
void selectMinPowerIdex(struct rtl8192cd_priv *priv)
{
	int i=0,idx, pwr_min = 0xff, pwr_tmp;
	unsigned int val32;
...
void PHY_RF6052SetOFDMTxPower(struct rtl8192cd_priv *priv, unsigned int channel)
{
	...
	if (GET_CHIP_VER(priv)==VERSION_8192D) {
		...
		if (priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G) {
			...
			priv->pshare->phw->OFDMTxAgcOffset_A[6] = 0;
			priv->pshare->phw->OFDMTxAgcOffset_A[7] = 0;
			priv->pshare->phw->OFDMTxAgcOffset_B[6] = 0;
			priv->pshare->phw->OFDMTxAgcOffset_B[7] = 0;
			...
			priv->pshare->phw->MCSTxAgcOffset_A[5] = 0;
			priv->pshare->phw->MCSTxAgcOffset_A[6] = 0;
			priv->pshare->phw->MCSTxAgcOffset_A[7] = 0;
			priv->pshare->phw->MCSTxAgcOffset_B[5] = 0;
			priv->pshare->phw->MCSTxAgcOffset_B[6] = 0;
			priv->pshare->phw->MCSTxAgcOffset_B[7] = 0;
			
			for (i=8; i<=12; i++) {
				priv->pshare->phw->MCSTxAgcOffset_A[i] = (priv->pshare->phw->MCSTxAgcOffset_A[i] < PWR_5G_DIFF)? 0: (priv->pshare->phw->MCSTxAgcOffset_A[i] - PWR_5G_DIFF);
				priv->pshare->phw->MCSTxAgcOffset_B[i] = (priv->pshare->phw->MCSTxAgcOffset_B[i] < PWR_5G_DIFF)? 0: (priv->pshare->phw->MCSTxAgcOffset_B[i] - PWR_5G_DIFF);
			}
			priv->pshare->phw->MCSTxAgcOffset_A[13] = 0;
			priv->pshare->phw->MCSTxAgcOffset_A[14] = 0;
			priv->pshare->phw->MCSTxAgcOffset_A[15] = 0;
			priv->pshare->phw->MCSTxAgcOffset_B[13] = 0;
			priv->pshare->phw->MCSTxAgcOffset_B[14] = 0;
			priv->pshare->phw->MCSTxAgcOffset_B[15] = 0;
#endif
		}
...
static void PHY_LCCalibrate(struct rtl8192cd_priv *priv)
{
	...
	unsigned int RF_Amode=0, RF_Bmode=0, LC_Cal;
	int isNormal;
	...
	if ((tmpReg & 0x70) != 0)				// Deal with contisuous TX case
		...
		RTL_W8(0x42, 0xFF);			// block all queues

	if ((tmpReg & 0x70) != 0)
	{
		// 1. Read original RF mode
		// Path-A
		RF_Amode = PHY_QueryRFReg(priv, RF92CD_PATH_A, 0x00, bMask12Bits, 1);

		// Path-B
		if (get_rf_mimo_mode(priv) != MIMO_1T1R)
			RF_Bmode = PHY_QueryRFReg(priv, RF92CD_PATH_B, 0x00, bMask12Bits, 1);

		// 2. Set RF mode = standby mode
		// Path-A
		PHY_SetRFReg(priv, RF92CD_PATH_A, 0x00, bMask12Bits, (RF_Amode&0x8FFFF)|0x10000);

		// Path-B
		if (get_rf_mimo_mode(priv) != MIMO_1T1R)
			PHY_SetRFReg(priv, RF92CD_PATH_B, 0x00, bMask12Bits, (RF_Bmode&0x8FFFF)|0x10000);
	}
	...
	if ((tmpReg & 0x70) != 0)	// Deal with contisuous TX case
	{
		// Path-A
		RTL_W8(0xd03, tmpReg);
		PHY_SetRFReg(priv, RF92CD_PATH_A, 0x00, bMask12Bits, RF_Amode);

		// Path-B
		if (get_rf_mimo_mode(priv) != MIMO_1T1R)
			PHY_SetRFReg(priv, RF92CD_PATH_B, 0x00, bMask12Bits, RF_Bmode);
	}
	else	// Deal with Packet TX case
	{
		RTL_W8(0x42, 0x00);
...
#ifdef CONFIG_RTL_92D_SUPPORT
	if (GET_CHIP_VER(priv) == VERSION_8192D) {
//		printk("[%s][%d]:[0x%x]\n", __FUNCTION__, __LINE__, priv->pshare->version_id);
...
#ifdef P2P_SUPPORT
	if(OPMODE&WIFI_P2P_SUPPORT){	// 0407
	...
	if (GET_CHIP_VER(priv) == VERSION_8192D){
		if (priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_5G  && !(OPMODE & WIFI_SITE_MONITOR)) {
			if (priv->pshare->DNC_on == 0){
				...
				if ((priv->pshare->F94_cnt + priv->pshare->Reg664_cnt)> 3000){
				...
				} else if ((priv->pshare->DNC_chk_cnt % 10)==1 && priv->pshare->DNC_chk == 1) {
...
#if defined(WIFI_11N_2040_COEXIST) && defined(BANDWIDTH_SWITCH)
void find_initialGain_Boundary(struct rtl8192cd_priv *priv)
{
	if (priv->pshare->rf_ft_var.use_ext_lna == 1) {
...
#if defined(WIFI_11N_2040_COEXIST) && defined(BANDWIDTH_SWITCH)
	if((priv->pshare->rf_ft_var.dnc_enable) && ( (priv->pshare->is_40m_bw==HT_CHANNEL_WIDTH_20) 
		|| (priv->pshare->_40m_staMap 
#ifdef STA_EXT		
	||	priv->pshare->_40m_staMap_ext 	
#endif
)))
#else
...
#if defined(WIFI_11N_2040_COEXIST) && defined(BANDWIDTH_SWITCH)
		if((priv->pshare->rssi_min > 40) && (priv->pshare->FA_total_cnt > (priv->pshare->threshold2*3)))
...
#if defined(CONFIG_RTL_92D_SUPPORT)
	  HostPCIe_SetPhyMdioWrite(RTL_USED_PCIE_SLOT, 0xf, 0x0f0f);
#else
...
#if defined(CONFIG_RTL_92D_SUPPORT)
	  HostPCIe_SetPhyMdioWrite(RTL_USED_PCIE_SLOT, 0xf, 0x0708);
#else
...
#if defined(__LINUX_2_6__)
    return request_irq(BSP_GPIO_ABCD_IRQ, gpio_wakeup_isr, IRQF_DISABLED, "rtk_gpio", dev);
#else
...
void PCIe_power_save_tasklet(unsigned long task_priv)
{
	...
	char in_out, L1_L2;
#if defined(CONFIG_RTL_92D_SUPPORT)
	unsigned int tmp32=0,status=0,page=0;
#else
...
//
void ReadEFuse(struct rtl8192cd_priv *priv, unsigned short _offset, int _size_byte, unsigned char *pbuf)
{
...
int isPGValueValid(struct rtl8192cd_priv *priv, unsigned char *hwinfo)
{
	int j;	
#ifdef CONFIG_RTL_92C_SUPPORT
	if (GET_CHIP_VER(priv)!=VERSION_8192D){
...
void ReadDeltaValFromEfuse(struct rtl8192cd_priv *priv)
{
	...
	priv->pmib->dot11RFEntry.deltaIQK = (hwinfo[EEPROM_92D_IQK_DELTA]&0x03);	//[7:0]
	priv->pmib->dot11RFEntry.deltaLCK = (hwinfo[EEPROM_92D_LCK_DELTA]&0x0C) >> 2;
}
...
void shadowMapWrite(struct rtl8192cd_priv *priv, int type, char *value, unsigned char *hwinfo)
{
