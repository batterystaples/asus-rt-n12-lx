 ...
 *  Utility routines
 *
 *  $Id: 8192cd_util.c,v 1.2 2011/05/16 09:57:01 edison_shih Exp $
 *
...
#ifdef SW_TX_QUEUE
	init_timer(&pstat->beq_timer);
	init_timer(&pstat->bkq_timer);
	init_timer(&pstat->viq_timer);
	init_timer(&pstat->voq_timer);

	skb_queue_head_init(&pstat->be_queue);
	skb_queue_head_init(&pstat->bk_queue);
	skb_queue_head_init(&pstat->vi_queue);
	skb_queue_head_init(&pstat->vo_queue);
#endif
...
#ifdef SW_TX_QUEUE
        while (skb_queue_len(&pstat->be_queue)) {
                pskb = skb_dequeue(&pstat->be_queue);
                rtl_kfree_skb(priv, pskb, _SKB_TX_);
        }
	while (skb_queue_len(&pstat->bk_queue)) {
                pskb = skb_dequeue(&pstat->bk_queue);
                rtl_kfree_skb(priv, pskb, _SKB_TX_);
        }
	while (skb_queue_len(&pstat->vi_queue)) {
                pskb = skb_dequeue(&pstat->vi_queue);
                rtl_kfree_skb(priv, pskb, _SKB_TX_);
        }
	while (skb_queue_len(&pstat->vo_queue)) {
                pskb = skb_dequeue(&pstat->vo_queue);
                rtl_kfree_skb(priv, pskb, _SKB_TX_);
        }
#endif
...
#ifdef SW_TX_QUEUE
	if (timer_pending(&pstat->beq_timer))
                del_timer_sync(&pstat->beq_timer);
	if (timer_pending(&pstat->bkq_timer))
                del_timer_sync(&pstat->bkq_timer);
	if (timer_pending(&pstat->viq_timer))
                del_timer_sync(&pstat->viq_timer);
	if (timer_pending(&pstat->voq_timer))
                del_timer_sync(&pstat->voq_timer);
#endif
#ifdef CONFIG_RTK_MESH
...
#ifdef TX_PATH_BY_RSSI
void remove_2r_sta(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
...
#ifdef TX_PATH_BY_RSSI
	remove_2r_sta(priv,pstat_chosen);
#endif
#ifdef STA_EXT
...
//			priv->STA_map &= ~(BIT(pstat->aid));
...
#endif
	{
		...
		/* 
		 * Releated to firmware's rate adaptive algorithm
		 * For 2R's STA, we set the init rate to MCS13 
		 */
		if (update_reg) {
...
void checkBandwidth(struct rtl8192cd_priv *priv)
{
	...
	if ((priv->pshare->CurrentChannelBW == HT_CHANNEL_WIDTH_20_40) 
		&& (priv->pshare->FA_total_cnt> priv->pshare->rf_ft_var.bws_Thd)
		&& ((RTL_R8(0xc50) & 0x7f) >= priv->pshare->FA_upper)	
		...
		||	priv->pshare->_40m_staMap_ext 	
				...
				priv->pshare->rf_ft_var.bws_triggered = 1;
		} 

		if( (priv->pshare->CurrentChannelBW == HT_CHANNEL_WIDTH_20)  &&
			(priv->pmib->dot11nConfigEntry.dot11nUse40M == HT_CHANNEL_WIDTH_20_40) && list_empty(phead)
			) {
				...
				SwBWMode(priv, priv->pshare->CurrentChannelBW, priv->pshare->offset_2nd_chan);
				SwChnl(priv, priv->pmib->dot11RFEntry.dot11channel, priv->pshare->offset_2nd_chan);
				priv->pshare->rf_ft_var.bws_triggered =0;
		}
}
#endif
...
static struct priv_skb_buf skb_buf[NUM_WLAN_IFACE][MAX_SKB_NUM];
static struct list_head skbbuf_list[NUM_WLAN_IFACE];
#ifdef CONFIG_WIRELESS_LAN_MODULE
static int skb_free_num[NUM_WLAN_IFACE] = {MAX_SKB_NUM, MAX_SKB_NUM};
#else
int skb_free_num[NUM_WLAN_IFACE] = {MAX_SKB_NUM, MAX_SKB_NUM};
#endif
...
void init_priv_skb_buf(struct rtl8192cd_priv *priv)
{
	...
	memset(skb_buf[idx], '\0', sizeof(struct priv_skb_buf)*MAX_SKB_NUM);
...
#ifdef SW_TX_QUEUE
	if ((AMPDU_ENABLE != 0) && (priv->pshare->rf_ft_var.swq_enable == 1))
	{
