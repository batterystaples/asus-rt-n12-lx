 ...
 * All rights reserved.                                                    
 * 
 * $Header: /home/user/PROJECT/Realtek/SDK2.4/rtl819x/linux-2.6.30/drivers/net/rtl819x/rtl_nic.c,v 1.2 2011/06/10 07:54:37 emily Exp $
 *
 * $Author: emily $
 *
...
#define CONFIG_RTL_LINKSTATE    //Emily
...
/* Emily */
static struct proc_dir_entry *phyPowerProc=NULL;
static int rtl865x_phyCtrlWriteProc(struct file *file, const char *buffer, unsigned long count, void *data);
static int rtl865x_phyCtrlReadProc(char *page, char **start, off_t off, int count, int *eof, void *data);
static int32 initPhyControl(void);
static void  exitPhyControl(void);
static int rtl865x_lanLinkWriteProc( struct file *filp, const char *buff,unsigned long len, void *data );
static int rtl865x_lanLinkReadProc( char *page, char **start, off_t off, int count, int *eof, void *data );
static int32 initLanLink(void);
static void  exitLanLink(void);
...
#define CONFIG_RTL_PHY_POWER_CTRL  //Emily
...
static int rtl865x_set_hwaddr(struct net_device *dev, void *addr)
{
	...
	{
		if(	(vlanconfig[i].vid==cp->id ) && (vlanconfig ->if_type==IF_ETHER) && 
...
static int32 initPhyControl(void)
{
	phyPowerProc = create_proc_entry("portControl", 0, NULL);	
	if(phyPowerProc)
	{
		phyPowerProc->read_proc = rtl865x_phyCtrlReadProc;
		phyPowerProc->write_proc = rtl865x_phyCtrlWriteProc;
	}
	memset(linkUpTime,0,sizeof(linkUpTime));
	init_linkup_time();
...
{
	if (phyPowerProc) {
		remove_proc_entry("portControl", phyPowerProc);				
		phyPowerProc = NULL;
	}
	del_timer(&s_timer);
}
...
static int32 initLanLink(void)
{
	phyPowerProc = create_proc_entry("LanLink", 0, NULL);	
	if(phyPowerProc)
	{
		phyPowerProc->read_proc = rtl865x_lanLinkReadProc;
		phyPowerProc->write_proc = rtl865x_lanLinkWriteProc;
	}
	memset(linkUpTime,0,sizeof(linkUpTime));
	init_linkup_time();
...
{
	if (phyPowerProc) {
		remove_proc_entry("LanLink", phyPowerProc);				
		phyPowerProc = NULL;
	}
	del_timer(&s_timer);
}	

static int rtl865x_phyCtrlReadProc(char *page, char **start, off_t off,
		     int count, int *eof, void *data)
{
	int len=0;
	return len;
...
static int rtl865x_phyCtrlWriteProc(struct file *file, const char *buffer,
		      unsigned long count, void *data)
{
	uint32 tmpBuf[32];
	uint32 uintVal;
	...
	if (buffer && !copy_from_user(tmpBuf, buffer, count))
	{
		...
		if (uintVal == WAN_POWEROFF)  // WAN port power off
		{  
			rtl865x_setPhyPowerOff(WAN_PORT_0);
			printk("Set WAN PhyPowerOff = %d!\n", uintVal);  
		
		}
		else if (uintVal == WAN_POWERON)  //WAN port power up
		{
			rtl865x_setPhyPowerOn(WAN_PORT_0);
			printk("Set WAN PhyPowerOn = %d!\n", uintVal); 
			
		}
		else if (uintVal == LAN_POWEROFF)  // LAN port power off
		{
			rtl865x_setPhyPowerOff(LAN_PORT_1);  //LAN port is PORT1~ PORT4
			rtl865x_setPhyPowerOff(LAN_PORT_2);
			rtl865x_setPhyPowerOff(LAN_PORT_3);
			rtl865x_setPhyPowerOff(LAN_PORT_4);
			printk("Set LAN PhyPowerOff = %d!\n", uintVal);
		}
		else if (uintVal == LAN_POWERON)  // LAN port power up
		{
			rtl865x_setPhyPowerOn(LAN_PORT_1);
			rtl865x_setPhyPowerOn(LAN_PORT_2);
			rtl865x_setPhyPowerOn(LAN_PORT_3);
			rtl865x_setPhyPowerOn(LAN_PORT_4);
			printk("Set LAN PhyPowerOn = %d!\n", uintVal);
		}
