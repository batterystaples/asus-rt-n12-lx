...
unsigned long linux_bin_size = 0;
// Alicia, 201103 }
...
#else
#define RTL_CFG_END          0x10000 /* 64KB */
static struct mtd_partition rtl8196_parts1[] = {
        {
                name:           "boot + cfg",
                size:           (RTL_CFG_END-0),
        ...
        {
                name:           "root fs",  
		size:	         (CONFIG_RTL_FLASH_SIZE - RTL_CFG_END),
                offset:         (RTL_CFG_END),
        },
        {
                name:           "linux + root fs",  
...
mod_init_t init_rtl8196_map(void)
{
	...
	my_sub_mtd[0]->read(my_sub_mtd[0], 0x10000, 16, numRead, &pHeader);
	printk(KERN_NOTICE "\n###len: 0x%x(%ld)", pHeader.len, pHeader.len);
	linux_bin_size = pHeader.len + 16;
	printk(KERN_NOTICE "\n###linux_bin_size: 0x%x(%ld)", linux_bin_size, linux_bin_size);

	map_cleanup();

	if (!strcmp(rtl8196_parts1[1].name, "root fs"))
	{
		printk(KERN_NOTICE "\n### find it\n");
		rtl8196_parts1[1].size = CONFIG_RTL_FLASH_SIZE - RTL_CFG_END - linux_bin_size;
		rtl8196_parts1[1].offset = RTL_CFG_END + linux_bin_size;
	}
