...
#if defined(CONFIG_RTL_8196B) || defined(CONFIG_RTL_8196C) || defined(CONFIG_RTL_8198)
static int test_gpio(void)
{
	FILE *fp;
	int i;
...
int main(int argc, char *argv[])
{
	...
	while(1) {  	  
		...
		fscanf(fp,"%d",&i);
		if (i==1&& (!isFileExist("/tmp/StartATEMode"))) {   //modify by Edison 2011.3.23
	    		printf("Going to Reload Default\n");
			system("flash default-sw");
			system("reboot");
		}
//		else if (i==2) {   
		else if (i==2 && (!isFileExist("/tmp/StartATEMode"))) {   //modify by Edison 2011.3.23
			system("flash default-sw");				  //add by Edison 2011.3.23
			system("reboot");				  //add by Edison 2011.3.23
		}
		fclose(fp); 
#endif
#ifdef CONFIG_RTL8186_KB
