...
//----------------------------------------------------------
	...
	timeout_end = uptime + server_config.auto_time;	//2011.06.28 Jerry
	while(1) { /* loop until universe collapses */
		...
		if (server_config.auto_time) {
			...
			tv.tv_sec = timeout_end - uptime();	//2011.06.28 Jerry
			tv.tv_usec = 0;
		}
	...
	if(hostname){	
		...
		case DHCPDECLINE:
			...
			if (lease) {
				...
				lease->expires = uptime() + server_config.decline_time;	//2011.06.28 Jerry
			}			
			break;
		case DHCPRELEASE:
			...
			if (lease){
				...
				lease->expires = uptime();	//2011.06.28 Jerry
		#ifdef STATIC_LEASE		 
