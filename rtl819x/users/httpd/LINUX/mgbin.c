 ...
 *
 *      $Id: mgbin.c,v 1.3 2011/05/27 09:21:25 emily Exp $
 *
 */
#include <string.h>
#include <stdio.h>
...
printf("=== ALL_HEADER = %s  SIGNATURE_LEN = %d === \n", ALL_HEADER, SIGNATURE_LEN);		
		}
#if 0
...
/*20110526 Emily add for utility fw header*/
			if (sector[i].type == SYS)
			{
						
					strcpy(checkmodel, CHIP_VENDOR);
					strcpy(checkmodel+MODEL_NAME_OFFSET, MODEL_NAME);
					write(fh_out, checkmodel, sizeof(checkmodel));
...
// Alicia, 201103 {
	if ((total - 16) > (CONFIG_RTL_FLASH_SIZE - 0x10000))  // not contain 16 bytes header of root.bin
	{ 
		printf("\nImage len exceed max size! len=0x%x.\n", total);
		exit(1);
	}
// Alicia, 201103 }
