...
void formVpnSetup(webs_t wp, char_t *path, char_t *query)
{
	...
	if ( strConnName[0] ) {
			if (strlen(strConnName) > (MAX_NAME_LEN-1)) {
				strcpy(tmpBuf, T("Error! Connection Name length too long."));
				goto setErr_vpn;
			}
	...
	if ( !string_to_dec(strTunnelId, &intVal) || intVal<1 || intVal>10) {
		strcpy(tmpBuf, T("Error! Invalid value of tunnel id"));
		goto setErr_vpn;
	}
	...
	if(entry.lcType == SUBNET_ADDR){ // subnet Address
		if(strLcMask[0]){
			if((intVal = mask2Len(strLcMask)) == -1){
				strcpy(tmpBuf, T("Error! Invalid value of Local IP mask"));
				goto setErr_vpn;
			}
		...
		for (i=1; i<=entryNum; i++) {
			*((char *)&checkEntry) = (char)i;
			...
			if ((entry.tunnelId != checkEntry.tunnelId)
				...
				strcpy(tmpBuf, T("Error! Cannot set Any Address in Remote Site more than one enabled-PSK entry."));
				goto setErr_vpn;
			}
		}
			...
			for (i=1; i<=entryNum; i++) {
				*((char *)&checkEntry) = (char)i;
				...
				if ((entry.tunnelId != checkEntry.tunnelId)
					...
					strcpy(tmpBuf, T("Error! Cannot set (PSK) Any Address in Remote Site more than one enabled-RSA entry"));
					goto setErr_vpn;
				}
			}
		}
	...
	&& strRtMask[0]){ // subnet Address
		if((intVal = mask2Len(strRtMask)) == -1){
			strcpy(tmpBuf, T("Error! Invalid value of RemoteIP mask"));
			goto setErr_vpn;
		}
	...
	if(entry.keyMode == IKE_MODE){ // Ike mode
		...
		if ( strPsKey[0]&& entry.authType ==0){ //PSK
			if (strlen(strPsKey) > (MAX_NAME_LEN-1)) {
				strcpy(tmpBuf, T("Error! Preshared Key length too long."));
				goto setErr_vpn;
			}
		...
		if(strSpi[0]){
				...
				strcpy(tmpBuf, T("Error! SPI length too long."));
				goto setErr_vpn;
			}
		...
		if ( strEncrKey[0] ) {
			if (strlen(strEncrKey) > (MAX_ENCRKEY_LEN-1)) {
				strcpy(tmpBuf, T("Error! Encryption Key length too long."));
				goto setErr_vpn;
			}
		...
		if ( strAuthKey[0] ) { // ESP/AH authenticaiton key 
			if (strlen(strAuthKey) > (MAX_AUTHKEY_LEN-1)) {
				strcpy(tmpBuf, T("Error! Authentication Key length too long."));
				goto setErr_vpn;
			}
	...
	*((char *)&entry) = (char)intVal;
	...
	if(curDhcp == DHCP_DISABLED || curDhcp == DHCP_CLIENT)
		...
		strcpy(tmpBuf, T("Can't set ipsec with pptp wan type."));		
		goto setErr_vpn;
	}
#if 0 //sc_yang
...
void formVpnConn(webs_t wp, char_t *path, char_t *query)
{
	...
	if(strTmp[0]){
		...
		if(sbuf.st_size > MAX_RSA_FILE_LEN){
		       strcpy(tmpBuf, T("RSA len exceed len"));
			goto setErr_vpnConn;
		}
		...
		if ( apmib_set( MIB_IPSEC_RSA_FILE, (void *)rsaBuf) == 0) {
		       strcpy(tmpBuf, T("Set RSA file error!"));
			goto setErr_vpnConn;
		}
		free(rsaBuf);
		close(fh);
