 ...
 *
 *      $Id: apmib.c,v 1.2 2011/05/16 01:58:09 jerry_jian Exp $
 *
 */
...
char *apmib_hwconf(void)
{
	...
	void *pDst = (void*)(HW_SETTING_OFFSET);
#endif
	...
	if(strcmp(compHeader.signature, COMP_HS_SIGNATURE) == 0 ) //check whether compress mib data
	{
...
//mib_display_tlv_content(HW_SETTING, expFile+sizeof(PARAM_HEADER_T), hsHeader.len);
		...
		{
			sprintf(&hsHeader.signature[TAG_LEN], "%02d", HW_SETTING_VER);
...
char *apmib_dsconf(void)
{
	...
	unsigned int expandLen=0, compLen=0;
	COMPRESS_MIB_HEADER_T compHeader;
	void *pDst = (void*)(DEFAULT_SETTING_OFFSET);
#endif
...
//fprintf(stderr,"\r\n __[%s-%u]",__FILE__,__LINE__);
	flash_read(&compHeader, pDst, sizeof(COMPRESS_MIB_HEADER_T));
//fprintf(stderr,"\r\n __[%s-%u]",__FILE__,__LINE__);	
	if(strcmp(compHeader.signature, COMP_DS_SIGNATURE) == 0 ) //check whether compress mib data
	{
		zipRate = compHeader.compRate;
		compLen = compHeader.compLen;
...
//mib_display_tlv_content(DEFAULT_SETTING, expFile+sizeof(PARAM_HEADER_T), dsHeader.len);
		...
		{
			sprintf(&dsHeader.signature[TAG_LEN], "%02d", DEFAULT_SETTING_VER);
...
//fprintf(stderr,"\r\n __[%s-%u]",__FILE__,__LINE__);
	flash_read(&compHeader, pDst, sizeof(COMPRESS_MIB_HEADER_T));
//fprintf(stderr,"\r\n __[%s-%u]",__FILE__,__LINE__);	
	if(strcmp(compHeader.signature, COMP_CS_SIGNATURE) == 0 ) //check whether compress mib data
	{
		zipRate = compHeader.compRate;
		compLen = compHeader.compLen;
...
//mib_display_tlv_content(CURRENT_SETTING, expFile+sizeof(PARAM_HEADER_T), csHeader.len);
		...
		{
			sprintf(&csHeader.signature[TAG_LEN], "%02d", CURRENT_SETTING_VER);
...
//mib_display_tlv_content(CURRENT_SETTING, data, tlv_content_len+1);			
	...
	if(mib_updateDef_compress_write(CURRENT_SETTING, data, &header) == 1)
	{
...
unsigned int mib_compress_write(CONFIG_DATA_T type, unsigned char *data)
{
	...
	if(compPtr != NULL && expPtr!= NULL)
	{
		...
		if(type == HW_SETTING)
			sprintf(compHeader.signature,"%s",COMP_HS_SIGNATURE);
		else if(type == DEFAULT_SETTING)
			sprintf(compHeader.signature,"%s",COMP_DS_SIGNATURE);
		else
			sprintf(compHeader.signature,"%s",COMP_CS_SIGNATURE);
...
//	fprintf(stderr,"%s %d\n",__FUNCTION__,__LINE__);
	...
	fh = open("/tmp/config.dat", O_RDWR|O_CREAT|O_TRUNC);	//2011.03.29 Jerry
#else
	fh = open("../web/config.dat", O_RDWR|O_CREAT|O_TRUNC);
#endif
	if (fh == -1) {
...
void mib_display_data_content(CONFIG_DATA_T type, unsigned char * pdata, unsigned int mib_data_len)
{
	...
	fprintf(stderr,"%p_",CHECKSUM(pdata, mib_data_len));
}
