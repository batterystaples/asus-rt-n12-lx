...
void formRoute(webs_t wp, char_t *path, char_t *query)
{
	...
	char_t *strIp, *strMask, *strGateway, *strRip, *tmpStr, *strMetric;
	char_t *strRip_Mode;
	char_t *strInterface;
	char_t *strIgmpProxyEnabled;	//2011.06.02 Jerry
	char *iface=NULL;
	char tmpBuf[100];
	...
	if(strRip[0]) {
		...
		if(intVal==0){
			...
			if ( apmib_set(MIB_NAT_ENABLED, (void *)&nat_mode) == 0) {
				strcpy(tmpBuf, T("\"Set NAT enabled error!\""));
				goto setErr_route;
			}
			enable_igmpproxy = 0;	
			if ( apmib_set(MIB_IGMP_PROXY_DISABLED, (void *)&enable_igmpproxy) == 0) {
				strcpy(tmpBuf, T("\"Set IGMP PROXY disabled error!\""));
				goto setErr_route;
			}
			goto setOk_route;
		}
		
		tmpStr = websGetVar(wp, T("nat_enabled"), T(""));  
		if(tmpStr[0]){
			if(tmpStr[0] == '0')
				nat_mode=1;
			else
		...
		strRip_Mode= websGetVar(wp, T("rip_tx"), T(""));
		if(strRip_Mode[0]) {
		...
		strRip_Mode= websGetVar(wp, T("rip_rx"), T(""));
		if(strRip_Mode[0]) {
		...
		if (apmib_set( MIB_RIP_LAN_RX, (void *)&ripmode_rx) == 0) {
			strcpy(tmpBuf, T("\"Set RIP LAN Rx error!\""));
				goto setErr_route;
			}
		if(nat_mode ==0){
			if ( apmib_set( MIB_RIP_WAN_RX, (void *)&ripmode_rx) == 0) {
					strcpy(tmpBuf, T("\"Set RIP WAN Rx error!\""));
				goto setErr_route;
			}
		}
		ok_msg = 1;
	...
	//2011.06.02 Jerry {
	strIgmpProxyEnabled = websGetVar(wp, T("igmpproxyEnabled"), T(""));
	if (strIgmpProxyEnabled[0])
	{
		if ( !gstrcmp(strIgmpProxyEnabled, T("ON")))
			enable_igmpproxy = 0;		
		else
			enable_igmpproxy = 1;
		apmib_set(MIB_IGMP_PROXY_DISABLED, (void *)&enable_igmpproxy);
	}
	//2011.06.02 Jerry }
	...
	if (strAddRoute[0]) {
		...
		strInterface=websGetVar(wp, T("iface"), T(""));
		strMetric=websGetVar(wp, T("metric"), T(""));
		...
		if(strcmp(iface, "ppp0")){
			...
			if (v1 && check_ip1 && check_ip2) {
				if ( (v1 & v3) != (v2 & v3) ) {
						if(entry.interface==1){
							strcpy(tmpBuf, T("Invalid Gateway address! It should be set within wan subnet."));
						}else if(entry.interface ==2)
							strcpy(tmpBuf, T("Invalid Gateway address! It should be set within wan physical subnet."));
						 else
							strcpy(tmpBuf, T("Invalid Gateway address! It should be set within lan subnet."));
						goto setErr_route;
					}
				}
		}
		...
		for(i=1; i <= entryNum ; i++) {
			*((char *)&checkentry) = (char)i;
			...
			if((*a1 & *a2) == (*b1&*b2)){
				sprintf(tmpBuf, T("Duplicate with entry %d!"),i);
				goto setErr_route;
			}
		}
#ifndef REBOOT_CHECK
...
/*
	...
	"<td align=center width=\"10%%\" bgcolor=\"#808080\">Select</td>\n"
      	...
      	"<td align=center width=\"10%%\" bgcolor=\"#808080\"><#RouterConfig_GWStaticIF_itemname#></td></tr>\n"
      	));
*/
	for (i=1; i<=entryNum; i++) {
		*((char *)&entry) = (char)i;
		...
		nBytesSent += websWrite(wp, T("<tr>"
			...
			"<td align=center width=\"23%%\" bgcolor=\"#C0C0C0\">%s</td>\n"
      			"<td align=center width=\"23%%\" bgcolor=\"#C0C0C0\">%s</td>\n"
      			"<td align=center width=\"23%%\" bgcolor=\"#C0C0C0\">%s</td>\n"
      			"<td align=center width=\"10%%\" bgcolor=\"#C0C0C0\">%d</td>\n"
      			"<td align=center width=\"10%%\" bgcolor=\"#C0C0C0\">%s</td></tr>\n"
      			),
				i, ip, netmask, gateway,entry.metric,(entry.interface?"WAN":"LAN"));
