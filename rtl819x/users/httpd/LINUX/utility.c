 ...
 *
 *      $Id: utility.c,v 1.3 2011/05/16 01:58:10 jerry_jian Exp $
 *
 */
...
dumpfile(struct ctfile_t *file,int filecount)
{
        int i;
...
dumpTorrents(struct torrent_t *torrentp, int count)
{
	int i;
	printf("dump Torrent\n");
	for(i=0;i<count;i++)
...
dumpClients(struct ctorrent_t *ctorrentp, int count)
{
	int i;
	printf("dump clients\n");
	for(i=0;i<count;i++)
...
WLAN_STATE_T updateWlanifState(char *wlanif_name)
{
	...
	sprintf(tmpStr,"/proc/%s/sta_info",wlanif_name);
	...
	if(lastWanState != wlanState)
	{
		...
		if(wlanState == WLAN_LINK)
			sprintf(tmpStr,"echo \"%s LINK\" > /var/wlan_state",wlanif_name);
		else
			sprintf(tmpStr,"echo \"%s NO_LINK\" > /var/wlan_state",wlanif_name);
