...
void Create_script(char *script_path, char *iface, int network, char *ipaddr, char *mask, char *gateway)
{
	...
	if(network==LAN_NETWORK){
		...
		write(fh, tmpbuf, strlen(tmpbuf));
		sprintf(tmpbuf, "ifconfig %s %s netmask %s\n", iface, ipaddr, mask);
		write(fh, tmpbuf, strlen(tmpbuf));
		sprintf(tmpbuf, "while route del default dev %s\n", iface);
		write(fh, tmpbuf, strlen(tmpbuf));
		sprintf(tmpbuf, "%s\n", "do :");
		write(fh, tmpbuf, strlen(tmpbuf));
		sprintf(tmpbuf, "%s\n", "done");
		write(fh, tmpbuf, strlen(tmpbuf));
		sprintf(tmpbuf, "route add -net default gw %s dev %s\n", gateway, iface);
		write(fh, tmpbuf, strlen(tmpbuf));
		sprintf(tmpbuf, "%s\n", "init.sh ap wlan_app");
		write(fh, tmpbuf, strlen(tmpbuf));
	}
	if(network==WAN_NETWORK){
		sprintf(tmpbuf, "%s", "#!/bin/sh\n");
		write(fh, tmpbuf, strlen(tmpbuf));
		sprintf(tmpbuf, "ifconfig %s 0.0.0.0\n", iface);
		write(fh, tmpbuf, strlen(tmpbuf));
	}
	close(fh);
}
