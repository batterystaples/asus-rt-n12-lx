...
void set_radvd()
{
	...
	if(isFileExist(RADVD_PID_FILE)){
		...
		if(radvdCfgParam.enabled == 1) {
			...
			RunSystemCmd(NULL_FILE, "/bin/radvd", "-C", "/var/radvd.conf", NULL_STR);
		} else {
			sprintf(tmpStr, "%d", pid);
			RunSystemCmd(NULL_FILE, "kill", "-9", tmpStr, NULL_STR);
		}
