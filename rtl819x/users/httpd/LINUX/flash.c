 ...
 *
 *      $Id: flash.c,v 1.13 2011/07/13 07:21:29 emily Exp $
 *
 */
...
//!CONFIG_RTL_8196B =>rtl8651c+rtl8190
		...
		for (idx=0; idx<NUM_WLAN_INTERFACE; idx++) {
			...
			memcpy(hwmib.wlan[idx].countrycode,"US",3);		
			hwmib.wlan[idx].regDomain = FCC;
			hwmib.wlan[idx].rfType = 10;
			hwmib.wlan[idx].ledType = 12;	//2011.04.01 Jerry
			hwmib.wlan[idx].xCap = 0;
			hwmib.wlan[idx].Ther = 0;
/*
...
*/
			...
			hwmib.wlan[idx].trswpape_C9 = 0xAA;
			hwmib.wlan[idx].trswpape_CC = 0xAF;
			
			hwmib.wlan[idx].target_pwr = 13;
...
//mib_display_tlv_content(HW_SETTING, data, header.len);				
	...
	strcpy(pMib->domainName,"ASUS");	//2011.04.13 Jerry
		...
		pMib->wlan[idx][i].p2p_op_channel = 11;		/*per channel that AP support */	
			...
			if (idx!=0)
...
#endif			
			  {
				...
				strcpy(pMib->wlan[idx][i].ssid, "ASUS" );	//2011.03.17 Jerry
				    else
					sprintf(pMib->wlan[idx][i].ssid, "RTK 11n AP %d",idx);
#endif
			  }				   
...
#endif
				{
					pMib->wlan[idx][i].channel = 0;	/*Edison 2011.5.30*/
			...
			if (i == 0)
			{
				...
				pMib->wlan[idx][i].channelbonding = 0;	//2011.04.13 Jerry
				pMib->wlan[idx][i].shortgiEnabled = 1;
				pMib->wlan[idx][i].fragThreshold=2346;	
				//pMib->wlan[idx][i].authType = AUTH_BOTH;
				pMib->wlan[idx][i].authType = AUTH_OPEN;	//2011.05.03 Jerry
				pMib->wlan[idx][i].inactivityTime = 30000; // 300 sec
				pMib->wlan[idx][i].dtimPeriod = 1;
				pMib->wlan[idx][i].wpaGroupRekeyTime = 86400;
				pMib->wlan[idx][i].wpaAuth = (unsigned char)WPA_AUTH_PSK;
				//pMib->wlan[idx][i].wpaCipher = (unsigned char)WPA_CIPHER_AES; //Keith	
				//pMib->wlan[idx][i].wpaCipher = (unsigned char)WPA_CIPHER_MIXED;	//2011.03.17 Jerry
				//pMib->wlan[idx][i].encrypt = (unsigned char)ENCRYPT_WPA;	//2011.03.17 Jerry
				pMib->wlan[idx][i].encrypt = (unsigned char)ENCRYPT_DISABLED;	//2011.05.03 Jerry
				pMib->wlan[idx][i].wep = WEP_DISABLED;	//2011.05.03 Jerry				
	...
	pMib->ipAddr[2] = 1;
	//pMib->ipAddr[3] = 254;
	pMib->ipAddr[3] = 1;	//2011.03.31 Jerry
	...
	pMib->dhcpClientStart[3] = 2;	/*Edison 2011.5.30*/

	pMib->dhcpClientEnd[0] = 192;
	pMib->dhcpClientEnd[1] = 168;
	pMib->dhcpClientEnd[2] = 1;
	pMib->dhcpClientEnd[3] = 254;	/*Edison 2011.5.30*/
	pMib->dhcpLease = 86400;	//2011.06.01 Jerry
	...
	strcpy(pMib->superName, "admin");	//2011.03.31 Jerry
	strcpy(pMib->superPassword, "admin");	//2011.03.31 Jerry

	pMib->scrlogEnabled=3;	//2011.03.31 Jerry
	pMib->refuse_eap=1;	//2011.04.20 Jerry
...
#endif
#ifdef HOME_GATEWAY
	pMib->qosAutoUplinkSpeed=0;	//2011.07.04 Jerry
	pMib->qosManualUplinkSpeed=512;
	pMib->qosAutoDownLinkSpeed=0;	//2011.07.04 Jerry
	pMib->qosManualDownLinkSpeed=512;
	
	pMib->vpnPassthruIPsecEnabled=1;
	pMib->vpnPassthruPPTPEnabled=1;
	pMib->vpnPassthruL2TPEnabled=1;
/*Edison 2011.5.5 modify wan pptp l2tp */
	pMib->wanIpAddr[0] = 0;
	pMib->wanIpAddr[1] = 0;
	pMib->wanIpAddr[2] = 0;
	pMib->wanIpAddr[3] = 0;

	pMib->wanSubnetMask[0] = 0;
	pMib->wanSubnetMask[1] = 0;
	pMib->wanSubnetMask[2] = 0;
	pMib->wanSubnetMask[3] = 0;

	pMib->wanDefaultGateway[0] = 0;
	pMib->wanDefaultGateway[1] = 0;
	pMib->wanDefaultGateway[2] = 0;
	pMib->wanDefaultGateway[3] = 0;

	pMib->wanDhcp = DHCP_CLIENT;
	pMib->dnsMode = DNS_AUTO;
	pMib->upnpEnabled = 1;	/* 0:off 1:on Edison 2011.5.30 */
	pMib->pppoeRelayEnabled = 0;  /* 0:off 1:on Emily 2011.7.12 */
	...
	pMib->pptpIpAddr[2]=0;
	pMib->pptpIpAddr[3]=0;
	pMib->pptpSubnetMask[0]=0;
	pMib->pptpSubnetMask[1]=0;
	pMib->pptpSubnetMask[2]=0;
	pMib->pptpSubnetMask[3]=0;
	pMib->pptpDefGw[0]=0;
	pMib->pptpDefGw[1]=0;
	pMib->pptpDefGw[2]=0;
	pMib->pptpDefGw[3]=0;
	pMib->pptpServerIpAddr[0]=0;
	pMib->pptpServerIpAddr[1]=0;
	pMib->pptpServerIpAddr[2]=0;
	pMib->pptpServerIpAddr[3]=0;			
	...
	pMib->l2tpIpAddr[2]=0;
	pMib->l2tpIpAddr[3]=0;
	pMib->l2tpSubnetMask[0]=0;
	pMib->l2tpSubnetMask[1]=0;
	pMib->l2tpSubnetMask[2]=0;
	pMib->l2tpSubnetMask[3]=0;
	pMib->l2tpDefGw[0]=0;
	pMib->l2tpDefGw[1]=0;
	pMib->l2tpDefGw[2]=0;
	pMib->l2tpDefGw[3]=0;
	pMib->l2tpServerIpAddr[0]=0;
	pMib->l2tpServerIpAddr[1]=0;
	pMib->l2tpServerIpAddr[2]=0;
	pMib->l2tpServerIpAddr[3]=0;	
	pMib->l2tpMtuSize = 1460; /* keith: add l2tp support. 20080515 */
	//pMib->L2tpwanIPMode = 1; /* keith: add l2tp support. 20080515 */
	pMib->L2tpwanIPMode = DYNAMIC_IP; /* keith: add l2tp support. 20080515 */
	...
	pMib->daylightsaveEnabled=0;
	strcpy(pMib->ntpTimeZone,"-8 4");
	strcpy(pMib->ntpSysTimeZone,"GMT+08:00");
	pMib->ntpServerIp1[0]=0;
	pMib->ntpServerIp1[1]=0;
	pMib->ntpServerIp1[2]=0;
	pMib->ntpServerIp1[3]=0;
/*
	...
	pMib->ntpServerIp2[3]=0;
*/
	pMib->ntpEnabled=1;	//2011.06.16 Jerry
	strcpy(pMib->ntpServerIp2,"time.nist.gov");		/*Edison 2011.5.30*/
	pMib->firewallEnabled=1;	/*Edison 2011.6.1*/
	pMib->webWanAccessPort = 8080;
	pMib->ddnsEnabled=0;
	pMib->ddnsType=0;
/*	strcpy(pMib->ddnsDomainName,"host.dyndns.org");	*/
	strcpy(pMib->ddnsDomainName,"");
...
static int initWlan(char *ifname)
{
	...
	if ((pmib->dot11StationConfigEntry.dot11DesiredSSIDLen == 3) &&
		...
		pmib->dot11StationConfigEntry.dot11SSIDtoScan[intVal2]='\0';
	}

	apmib_get(MIB_WLAN_MODE, (void *)&mode);
	//if (mode == 1) {
		// client mode
	if (mode == CLIENT_MODE)			/*client mode*/ 
	{
...
static int updateWscConf(char *in, char *out, int genpin, char *wlanif_name)
{
	...
	//	intVal=1;
	//else if(NUM_WLAN_INTERFACE==2)
	//	intVal=2;
	//else
	//	intVal=1;
	//WRITE_WSC_PARAM(ptr, tmpbuf, "wlan_inter_num = %d\n", intVal);			
