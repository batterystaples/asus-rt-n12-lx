	...
	if(auth->eapType == EAP_MD5){		
		...
		while(fgets(buffer, sizeof(buffer), inFp) != NULL){
			...
			if(isFileExist(RS_USER_CERT) == 1){
				ptr=strstr(buffer, "#eap_peap_user_cert#");
				if(ptr != NULL){
					//To rm "#eap_peap_user_cert#"
					memset(tmpBuf, 0, sizeof(tmpBuf));
					ptr += strlen("#eap_peap_user_cert#");
					strcpy(tmpBuf, ptr);
				...
				ptr=strstr(buffer, "#eap_peap_user_key#");
				if(ptr != NULL){
					//To rm "#eap_peap_user_key#"
					memset(tmpBuf, 0, sizeof(tmpBuf));
					ptr += strlen("#eap_peap_user_key#");
					strcpy(tmpBuf, ptr);
			...
			if(isFileExist(RS_ROOT_CERT) == 1){
				ptr=strstr(buffer, "#eap_peap_root_cert#");
				if(ptr != NULL){
					//To rm "#eap_peap_root_cert#"
					memset(tmpBuf, 0, sizeof(tmpBuf));
					ptr += strlen("#eap_peap_root_cert#");
					strcpy(tmpBuf, ptr);
...
// david ----------------------
	...
	if(auth->currentRole ==  role_Supplicant_infra){
		...
		if(prepareClientMode1xConf(&RTLAuthenticator, XSUP_CONF_FILE) == FAILED){
...
//				not_associated,xsup_inited);//Added for test
				...
				else
				{
					...
					static char *xsup_argv[]= { "xsuplicant", "-i", "wlan0", "-c", "/var/1x/1x.conf", "-d", "0" };
