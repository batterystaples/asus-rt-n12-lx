...
static void WFAGetDeviceInfo(struct upnphttp * h)
{
	...
	if(pCtx->is_ap){		
		if(pCtx->wlan0_wsc_disabled!=1)	{
			...
			SUM_DEBUG("COME_FROM_WLAN0\n");
		}else if(pCtx->wlan1_wsc_disabled!=1){
			pCtx->InterFaceComeIn = COME_FROM_WLAN1 ;
			SUM_DEBUG("COME_FROM_WLAN1\n");	
		}else{
...
static void PsimplecfgDeviceHandleSubscriptionRequest(struct upnp_subscription_element *sub) 
{
	...
	for(i=0 ; i<pCtx->TotalSubscriptions ; i++ ){
		_DEBUG_PRINT("Subscription Sid(%d) = %s ; used=%d\n", i , pCtx->upnp_subscription_info[i].Sid , pCtx->upnp_subscription_info[i].used);		
	...
	if (subscription) {
		if (subscription->used) {
			...
			if (pCtx->setSelectedRegTimeout == 0){
				...
				DEBUG_PRINT("ReNewal Subscription fail setSelectReg timeout!\n");
			}
